      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            DeFi Agent
            <span className="text-xs px-2 py-0.5 rounded bg-jpm/15 text-gold border border-uni/25 font-normal">Beta</span>
          </h1>
          <p className="text-sm text-gray-500">Your DeFi financial advisor — powered by Venice AI</p>
        </div>
        <div className="flex items-center gap-1.5 text-xs text-gold">
          <div className="w-2 h-2 rounded-full bg-uni" />Venice AI · Zero Retention
        </div>
      </div>

      {/* Sub-tabs */}
      <div className="flex gap-1 mb-4 border-b border-navy-600 pb-px">
        <button onClick={() => setTab('chat')}
          className={`px-4 py-2 text-sm font-medium transition border-b-2 -mb-px ${tab === 'chat' ? 'border-uni text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}>
          Chat with Advisor
        </button>
        <button onClick={() => setTab('connect')}
          className={`px-4 py-2 text-sm font-medium transition border-b-2 -mb-px ${tab === 'connect' ? 'border-uni text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}>
          Connect Your Agent
        </button>
      </div>
      ...
      {tab === 'connect' ? (
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pr-1">
          ...
          <div className="bg-navy-800 border border-navy-600 rounded-xl p-6">
            ...
            <div className="flex items-center justify-center gap-3 mb-6 py-4">
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-jpm/20 border border-jpm/30 flex items-center justify-center mx-auto mb-2">
                  <span className="text-sm font-bold text-uni">You</span>
                </div>
              ...
              <svg className="w-5 h-5 text-gray-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-jpm/20 border border-uni/30 flex items-center justify-center mx-auto mb-2">
                  <span className="text-sm font-bold text-gold">DeFi</span>
                </div>
              ...
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center mx-auto mb-2">
                  <span className="text-[10px] font-bold text-emerald-400">Chain</span>
                </div>
          ...
          <div className="bg-navy-800 border border-navy-600 rounded-xl p-6">
            ...
            <div className="bg-navy-950 rounded-lg p-4 font-mono text-sm text-gray-300 relative group">
              <div className="break-all">{baseUrl}/api/skill</div>
              <button onClick={() => { navigator.clipboard.writeText(`${baseUrl}/api/skill`); }}
                className="absolute top-2 right-2 bg-navy-700 hover:bg-navy-600 text-gray-400 hover:text-white px-2 py-1 rounded text-xs transition opacity-0 group-hover:opacity-100">
                Copy
              </button>
            </div>
          ...
          <div className="bg-navy-800 border border-navy-600 rounded-xl p-6">
            ...
            <div className="bg-navy-950 rounded-lg p-4 text-sm text-gray-300 relative group">
        ...
        <>
          <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-1">
            {messages.length === 0 && (
              <div className="text-center py-12">
                <div className="w-16 h-16 rounded-2xl bg-jpm/20 flex items-center justify-center mx-auto mb-6">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {['What should I do with my vault?', 'Explain the risks of each strategy', 'How do guardrails protect me?', 'How does agent-to-agent work?'].map(q => (
                    <button key={q} onClick={() => { setInput(q); }}
                      className="text-xs bg-navy-700 hover:bg-navy-600 text-gray-300 px-3 py-2 rounded-lg transition">
                      {q}
                    </button>
                  ))}
                </div>
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-navy-700 rounded-xl px-4 py-3 text-sm text-gray-400">
          ...
          <div className="flex gap-2">
            <input value={input} onChange={e => setInput(e.target.value)}
              ...
              className="flex-1 bg-navy-700 border border-navy-600 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-jpm" />
            <button onClick={send} disabled={loading || !input.trim()}
              className="bg-jpm hover:bg-jpm disabled:bg-navy-700 disabled:text-gray-500 px-5 py-3 rounded-lg text-sm font-medium transition">
              Send
            </button>
          </div>
        </>
