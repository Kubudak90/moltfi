    ...
    <div className="relative">
      <button
        ...
        className={`flex items-center gap-1.5 border rounded-full px-3 py-1 text-xs transition ${
          privateMode
            ? 'bg-jpm/10 border-uni/20 text-gold-light hover:bg-jpm/20'
            : 'bg-navy-700/50 border-navy-600 text-gray-500 hover:bg-navy-700'
        }`}
      >
        ...
        {privateMode && <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />}
      </button>

      {showDetail && (
        <div className="absolute right-0 mt-2 w-80 bg-navy-800 border border-navy-600 rounded-xl shadow-xl p-5 z-50">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className={`ml-auto flex items-center gap-1 text-xs ${privateMode ? 'text-emerald-400' : 'text-gray-500'}`}>
              <div className={`w-1.5 h-1.5 rounded-full ${privateMode ? 'bg-emerald-400' : 'bg-gray-600'}`} />
          ...
          <div className="mt-4 pt-3 border-t border-navy-600 flex items-center justify-between">
            <div className="text-xs text-gray-600">
              Model: <span className="text-gray-400 font-mono">llama-3.3-70b</span>
            </div>
            <a href="https://venice.ai" target="_blank" rel="noopener" className="text-xs text-gold hover:underline">
